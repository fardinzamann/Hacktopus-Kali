    ...
    for payload in payloads:
        ...
        try:
            ...
            if "error" in response.text.lower():
                print(f"⚠️ Potential SQL Injection detected with payload: {payload}")
        except requests.exceptions.RequestException as e:
    ...
    for payload in payloads:
        ...
        try:
            ...
            if payload in response.text:
                print(f"⚠️ Potential XSS vulnerability detected with payload: {payload}")
        except requests.exceptions.RequestException as e:
            print(f"❌ Error occurred with payload {payload}: {e}")
...
def check_sensitive_data_exposure(target):
    ...
    if target.startswith("http://"):
        print(f"⚠️ Warning: Sensitive data might be exposed over HTTP, consider using HTTPS.")
    else:
